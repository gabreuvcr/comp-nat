...
# #primeiro conjunto de parametros
# QTD_FORMIGAS = 100
# ITERACOES = 50
# FER_INICIAL = 0.1
# TX_EVA = 0.1
...
#segunda conjunto de parametros
QTD_FORMIGAS = 100
ITERACOES = 25
FER_INICIAL = 0.01
TX_EVA = 0.25
...
class Colonia:
    ...
    def calcula_makespan(self, caminho):
        ...
        for adjacente in grafo_di[INICIO]:
            tempo[adjacente] = self.custo(adjacente)
        for vertice in caminho:
    ...
    def execucao(self):
        ...
        print(f"Melhor makespan: {melhor_formiga.makespan}")
...
start = time.time()
maquinas, tempos, qtd_trab, qtd_maq = arquivo_entrada("files/la01.txt")
colonia = Colonia(maquinas, tempos, qtd_trab, qtd_maq)
print(f"\n{time.time() - start:.4f} segundos")
